...
- (UIView *)addCustomView:(UIView *)customView
{
    ...
    CGPoint origin = customView.frame.origin;
    CGSize size = customView.frame.size;
//    origin.x = (customView.superview.frame.size.width - size.width)/2;
//    origin.y = (customView.superview.frame.size.height - size.height)/2;
    
//    origin.x = (self.windowWidth - size.width)/2;
//    origin.y = (self.windowHeight - size.height)/2;
    
//    origin.x=300;
//    
//    origin.y=200;
    
    customView.frame = (CGRect){origin,size};
    NSLog(@"frame:{%f,%f,%f,%f}",origin.x,origin.y,size.width,size.height);
    NSLog(@"super frame:{%f,%f,%f,%f}",customView.superview.frame.origin.x,
          customView.superview.frame.origin.y,
          customView.superview.frame.size.width,
          customView.superview.frame.size.height);

    
    [_contentView addSubview:customView];
    
    
//    [self.SCLAlertWindow addSubview:customView];
    [_customViews addObject:customView];
    
    NSLog(@"_contentView frame:{%f,%f,%f,%f}",_contentView.frame.origin.x,
